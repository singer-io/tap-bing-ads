            ...
            try:
                ...
                if hasattr(e.fault.detail, 'ApiFaultDetail'):
                    ...
                    if any(invalid_date_range_end_errors):
                        raise InvalidDateRangeEnd(
                            invalid_date_range_end_errors) from e
                    LOGGER.info(
                        'Caught exception for account: {}'.format(account_id))
                    raise Exception(operation_errors) from e
                if hasattr(e.fault.detail, 'AdApiFaultDetail'):
                    raise Exception(
                        e.fault.detail.AdApiFaultDetail.Errors) from e
...
def get_stream_def(stream_name, schema, stream_metadata=None, pks=None, replication_key=None):
    ...
    if stream_metadata:
        ...
        stream_def['metadata'] = list(map(
            lambda field: {"metadata": {"inclusion": "available"},
...
def discover_core_objects():
    ...
    core_object_streams.append(get_stream_def(
        'campaigns', campaign_schema, pks=['Id']))

    ad_group_schema = get_core_schema(client, 'AdGroup')
    core_object_streams.append(get_stream_def(
        'ad_groups', ad_group_schema, pks=['Id']))
...
def metadata_fn(report_name, field, required_fields):
    if field in required_fields:
        ...
        mdata = {"metadata": {"inclusion": "available"},
                 "breadcrumb": ["properties", field]}

    if EXCLUSIONS.get(report_name):
        if field in EXCLUSIONS[report_name]['Attributes']:
            mdata['metadata']['fieldExclusions'] = [['properties', p]
        ...
        if field in EXCLUSIONS[report_name]['ImpressionSharePerformanceStatistics']:
            mdata['metadata']['fieldExclusions'] = [['properties', p]
                                                    for p in EXCLUSIONS[report_name]['Attributes']]
...
def do_discover(account_ids):
    ...
    json.dump({'streams': core_object_streams +
               report_streams}, sys.stdout, indent=2)


def check_for_invalid_selections(prop, mdata, invalid_selections):
    field_exclusions = metadata.get(
        mdata, ('properties', prop), 'fieldExclusions')
    ...
    if field_exclusions and is_prop_selected:
        for exclusion in field_exclusions:
            is_exclusion_selected = metadata.get(
                mdata, tuple(exclusion), 'selected')
    ...
    if 'Campaign' in response_dict:
        ...
        if 'campaigns' in selected_streams:
            selected_fields = get_selected_fields(
                selected_streams['campaigns'])
            singer.write_schema('campaigns', get_core_schema(
                client, 'Campaign'), ['Id'])
    ...
    for campaign_id in campaign_ids:
        ...
        if 'AdGroup' in response_dict:
            ...
            if 'ad_groups' in selected_streams:
                ...
                singer.write_schema('ad_groups', get_core_schema(
                    client, 'AdGroup'), ['Id'])
...
def log_retry_attempt(details):
    LOGGER.info('Retrieving report timed out, triggering retry #%d',
                details.get('tries'))
